---
layout  : wiki
title   : vimwiki+github.io 시작하기
summary : 이종립 선생님 버전 을 이용한 vimwiki+github.io를 통한 개인 위키 시작.
date    : 2026-01-14 14:27:11 +0900
updated : 2026-01-14 17:26:49 +0900
tag     : log
toc     : true
public  : true
parent  : [[index]]
latex   : false
resource: b0b032a7-f51c-4620-b58d-f53c15eea3bb
---
* TOC
{:toc}

# vimwiki+github.io 시작하기

이종립 선생님이 만드신 wiki 를 설치하고, 내 정보들로 설정만 바꿈.

* 우선 vimscript가 아닌 lua로 설정을 처리하고,
* ruby 버전을 [git pahge 의 ruby 3.3.4](https://pages.github.com/versions.json)에 맞춰 변경함.
* 익숙한 conda 환경 내에서 vimwiki를 처리하게 설치.

정말 똑똑하신 분들이 많고 그 분들 덕분에 많은 걸 누리는 듯...

## 설정에 대한 참고 자료

### 원본

다음은 모두 이종립 선생님이 작성하신 자료들임.

* [원본 : skeleton-blog](https://github.com/johngrib/johngrib-jekyll-skeleton)
* [이종립 선생님의 설명자료](https://johngrib.github.io/wiki/my-wiki/)
* [vimwiki 사용법](https://johngrib.github.io/wiki/vimwiki/)
* [기타 설정 내용](https://johngrib.github.io/wiki/blog/)

### 이 블로그 설정 문서

* [ds31x's vimwiki+github.io 설치 작업 로그](https://ds31x.tistory.com/604)

## 이미지 테스트.

![](https://github.com/user-attachments/assets/7153c3a4-533a-4cd1-83b0-505710af66ab)
